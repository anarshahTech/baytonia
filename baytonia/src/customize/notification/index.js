...
import styles from '../../core/base/components/notification/styles';
...
class Notification extends React.Component {
    ...
    sentUrlForWebApp(noti) {
        if (noti.is_onesignal) {
            if (noti.payload.additionalData) {
                let additionalData = noti.payload.additionalData;
                if (additionalData.url_redirect && additionalData.url_redirect.length > 0) {
                    return additionalData.url_redirect;
                }
            }
        }
        return '';
                        ...
                        <View style={styles.buttonContainer}>
                            ...
                            <Button transparent style={styles.button} onPress={() => {
                                this.props.storeData('actions', [
                                    {
                                        type: 'showNotification', data: {
                                            show: false,
                                            data: {},
                                        }
                                    },
                                    {
                                        type: 'customize_noti', data: {
                                            redirect: this.sentUrlForWebApp(noti)
                                        }
                                    },
                                ]);
                            }}>
